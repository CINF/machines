...
class DI2008(object):
    ...
    def config_scan_list(self):
        ...
        self.comm('slist 7 29696') # K-type TC on channel 7 ###
    ...
    def set_dac(self, channel, voltage):
        """
        Set the DAC (analog output) on the DI-2008.

        Parameters:
            channel (int): 0 for DAC0, 1 for DAC1
            voltage (float): Output voltage (0.0 to 10.0 V)
        """
        if channel not in [0, 1]:
            raise ValueError("Channel must be 0 (DAC0) or 1 (DAC1)")
        if not (0.0 <= voltage <= 10.0):
            raise ValueError("Voltage must be in range 0.0 to 10.0 V")
        value = int((voltage / 10.0) * 65535)
        self.comm(f'dac {channel} {value}')

    def set_channels_out_test(self):
        self.comm('endo 20')
        self.comm('dout 4')
        print('>>> set outputs')
...
class Reader(threading.Thread):
    ...
    def run(self):
        ...
        self.dev.set_dac(0,2)
        self.dev.set_channels_out_test()
        self.dev.start()
        t0 = time.time()
...
class Printer(threading.Thread):
    ...
    def print_data(self):
        ...
        for i in range(8):
            ...
            if i == 7:
                pass
                # Your new voltage input
                #try:
                #    self.win.addstr(8+i, 0, '{}: {:.3f} mV                   '.format(i+1, channel))
                #except (TypeError, ValueError):
                #    self.win.addstr(8+i, 0, '{}: invalid data                '.format(i+1))
            else:
                # Your existing thermocouple channels
                cjc = self.dev.cjc[i]
                if isinstance(channel, str):
                    formatstring = '{}: {}      ({:.3f} C)     '
                else:
