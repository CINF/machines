...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run DI2008 data acquisition.')
    parser.add_argument(
        '--high-frequency',
        action='store_true',
        help='Use high-frequency mode (timeout=0.25s for last channel instead of 5s)'
    )
    ...
    # Adjust timeout for last channel based on CLI flag
    weight_cell_timeout = 0.25 if args.high_frequency else 5
    
    criterium_checker = LoggingCriteriumChecker(
        codenames=codenames,
        types=['lin']*len(codenames),
        criteria=[0.5]*len(codenames),
        time_outs=[300, 300, 300, 300, 300, 300, 300, weight_cell_timeout], #hrogr can update, below 380 ms
    )
